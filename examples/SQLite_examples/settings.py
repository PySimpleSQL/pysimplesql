...
-- There is nothing special about the key and value column names, they can literally be anything allowed by SQLite.
...
# When using Form.field() to create entries based on key/value pairs, it just uses an extended syntax.
# Where Form.field('Settings.value') would return the value column from the Settings table FOR THE CURRENT RECORD,
# the extended syntax of Form.field('Settings.value?key=first_name') will return the value column from the Settings
...
layout = [
    ...
    # We can use get_keyed_value() to retrieve values from keys in the database.  We are using it to set tooltips.
    [sg.Text('')],
    ...
    [ss.actions('Settings', default=False, save=True)]
]
