...
with tqdm(total=1, desc="Building Docker image") as pbar:
    image, build_logs = client.images.build(
        path=".", dockerfile="Dockerfile", tag="pysimplesql/examples:mysql"
    )
    pbar.update()
...
with tqdm(total=1, desc="Committing container") as pbar:
    image = container.commit(repository="pysimplesql/examples", tag="mysql")
    pbar.update()
...
with tqdm(total=1, desc="Pushing image to Docker Hub") as pbar:
    client.images.push("pysimplesql/examples", "mysql")
    pbar.update()
