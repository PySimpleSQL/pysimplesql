...
def record_controls(table, edit_protect=False):
    layout = [
        sg.T(f'{table}:')
    ]
    if edit_protect:
        layout += [sg.B('', key=f'btnEditProtect', size=(1, 1), image_data=edit_16,metadata=True)]  # disabled=True
...
def record_actions(table,save=True):
    layout = [
        sg.B('', key=f'Event.{table}.Insert', size=(1, 1), image_data=add_16),
        sg.B('', key=f'Event.{table}.Delete', size=(1, 1), image_data=delete_16),
    ]
    if save:
        layout += [sg.B('', key=f'btnSaveRecord', size=(1, 1), image_data=save_16)]
    return layout


def record_buttons(table, edit_protect=False):
    layout = record_controls(table, edit_protect)+record_actions(table)
    return layout
...
def set_control_size(w,h):
    """Sets the defualt text (label) size when @record is used.  The size parameter of @record will overrule this"""
...
def record(table, field, control=sg.I, size=None,  name='' ):
    """size overwrites the default control size"""
    global _default_text_size
    global _default_control_size
